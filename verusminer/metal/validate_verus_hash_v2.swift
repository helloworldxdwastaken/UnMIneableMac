...
func runTest(label: String, seed: UInt64, len: Int, traced: Bool = false) -> Bool {
    ...
    if traced && !ok {
        ...
        print("    intermediate (opt CPU): \(cpuInt == gpuInt ? "✓" : "✗")  CPU=\(hex(cpuInt))")
        print("                                          GPU=\(hex(gpuInt))")
        print("    intermediate (portable CPU): \(portInt == gpuInt ? "✓ GPU matches portable" : "✗")  port=\(hex(portInt))")
